...
############################################
# Author: SDW
# Purpose: Provides local configuration of LDAP settings
#          This script is complementary to setup_ldap.sh
#          it will not provide comprehensive LDAP setup
############################################
...
if [[ $DMZ == FALSE ]]; then
   ...
   $WGET --quiet http://${LDAP_MASTER_SERVER}/0uMxWccP3EtxmU2xVJV5Hqjl4/setup_ldap.sh -O $NETVERTMP
   ...
   if [[ -s "$NETVERTMP" ]]; then
      ...
      if [[ -n $THISSER ]] && [[ -n $THATSER ]] && [[ $THATSER -gt $THISSER ]]; then
         ...
         #echo "chmod +x $NETVERTMP;/bin/mv $NETVERTMP /opt/configure_ldap.sh; /opt/configure_ldap.sh;" | /bin/bash &
...
if [[ -z `echo $PRODUCT | $EGREP 'RHEL|RHES|RHAS'` ]] || [[ $RELEASE -le 3 ]]; then
...
if [[ -z $LDAP_SERVER_LIST ]]; then
   LDAPCSV=/tmp/lst.csv
   $WGET --quiet http://${LDAP_MASTER_SERVER}/0uMxWccP3EtxmU2xVJV5Hqjl4/replids.csv -O $LDAPCSV
   CSVRESULT=$?
   if [[ $CSVRESULT == 0 ]]; then
...
for s in $LDAP_SERVER_LIST; do
   ...
   if [[ -z `/usr/bin/ldapsearch -x -o nettimeout=7 -h $s 2>&1 | $GREP "Can't contact LDAP server"` ]]; then
...
$WGET --quiet http://${best}/0uMxWccP3EtxmU2xVJV5Hqjl4/cacert.asc -O /etc/openldap/cacerts/cacert.asc
if [[ -s /etc/openldap/cacerts/cacert.asc ]]; then
   ...
   echo ""
   echo "Please be sure the file is available and reachable from"
   echo "this server.  This server needs to be able to contact"
   echo "the LDAP servers on port 80."
   echo ""
   echo "If it is not possible to download the file directly"
   echo "to this server via http, you can also manually copy"
   echo "the \"cacert.asc\" file from the directory server and"
   echo "place it in \"/etc/openldap/cacerts/\" and re-run this script."
   exit 9
fi
...
if [[ -z `$LDAP_SEARCH -b $NG_BASE "(&(objectClass=nisnetgroup)(cn=${MNG}))" cn | $GREP ^cn:` ]]; then
   ...
   if [[ -n `$LDAP_SEARCH -b $NG_BASE "(Description=:::${SERIAL}::*)" | $EGREP -v '^#|^ |^$|^search:|^result:'` ]]; then
      ...
      # If no duplicate names or serials were found then we'll tentatively set the operation to ADD
      OPERATION=ADD
   fi
   ...
   EXISTSERIAL=`echo $EXISTDESC | awk -F'::' '{print $1}' | sed 's/ *$//g'`
   EXISTHN=`echo $EXISTDESC | awk -F'::' '{print $2}' | sed 's/ *$//g'`
   EXISTPN=`echo $EXISTDESC | awk -F'::' '{print $3}' | sed 's/ *$//g'`
   EXISTDATE=`echo $EXISTDESC | awk -F'::' '{print $4}'`
   EXISTIP=`echo $EXISTDESC | awk -F'::' '{print $5}'`
...
chattr -i /etc/group
chattr -i /etc/shadow
echo "Applying authconfig settings for "
...
if [[ $RELEASE == 7 ]]; then
   ...
   echo "RHEL 6"
   echo "/usr/sbin/authconfig --enableshadow --passalgo=sha512 --enablemd5 --disablenis --enableldap --enableldapauth --ldapserver=$OSL --ldapbasedn='"$LDAP_BASE"' --enableldaptls --enableforcelegacy --enablecache --enablelocauthorize --enablepamaccess --enablemkhomedir --updateall" | /bin/bash
