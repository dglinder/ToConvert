...
if [[ $ADDLOCAL != TRUE ]]
then
	echo Verifying DNS resolution of $SSDNSNAME.....
	SSRESCHECK=`host oma00rhs01.svc.west.com 2>&1`
	if [[ $? -ne 0 ]]
	then
		ADDLOCAL=TRUE
	fi
...
then
	...
	if [[ -z $HOSTCHECK ]]
	then
		echo "DNS resolution failed, appending \"$SSIPADDR $SSDNSNAME\" to /etc/hosts before proceeding."
        	echo "$SSIPADDR $SSDNSNAME" >> /etc/hosts
	fi
else
        echo DNS resolution is good: $SSRESCHECK
fi
...
/usr/sbin/rhnreg_ks --force --profilename=${PROFILENAME} --serverUrl=https://${SSDNSNAME}/XMLRPC --sslCACert=/usr/share/rhn/RHN-ORG-TRUSTED-SSL-CERT --activationkey=${PRIMARYKEY},${TRACKINGKEY}
RESULT=$?
