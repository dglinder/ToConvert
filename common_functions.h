...
#------------------------------------------------------
# Function f_DEBUG
# Purpose: Enhanced logging function
#
# Usage: f_DEBUG Message to display and log with date and line number.
#  Output: 2016-06-13 12:43:51 [L:00062] Message to display...
#
# Usage: f_DEBUG "This is a
#        multi-line log entry
#            with indented lines."
#  Output: 2016-06-13 12:44:02 [L:00062] This is a
#          2016-06-13 12:44:02 [L:00062] multi-line log entry.
#          2016-06-13 12:44:02 [L:00062]     with indented lines.
#
#------------------------------------------------------
f_DEBUG () {
# Write usable log messages with line numbers to $LOGPATH.
# Add enahnced call stack trace output by setting $ENHANCED_DEBUG to 1
	if [ -z "${LOGPATH}" ] ; then
	...
	SN=$(basename $0 .sh)
		LOGFILE=${LOGPATH}/${SN}.log

		f_Get_Stack () {
		# Get the series of calling funcitons and scripts for enhanced output.
		# This function based on code by akostadinov as submitted
		# to StackOverflow.com - http://stackoverflow.com/a/17734099/187426
			STACK=""
			local i message="${1:-""}"
			...
			for (( i=1; i<$stack_size; i++ )); do
				...
				STACK+=$(printf "\n...at: src:\"$src\" func:\"$func\" ln:\"$linen\"")
			done
			STACK="${message}${STACK}"
		}

		if [ $ENHANCED_DEBUG -gt 0 ] ; then
			f_Get_Stack "$@"
		else
			# Get the line number of the calling function/line.
			STACK="$@"
		fi

	# All output has the line number so grab it once.
	lnum=$(printf [L:%05i] ${BASH_LINENO[0]})

	# Make sure the log path directory is created.
	if [[ ! -d ${LOGPATH} ]]; then mkdir -p ${LOGPATH} ; fi

	# Use IFS='' to ensure leading whitespace in log message is preserved.
	echo "${STACK}" | while IFS='' read -r line; do
		# Save date/time and line information to the file.
		date +"%Y-%0m-%0d %0H:%0M:%0S $lnum $line" >> $LOGFILE
		# Keep the output to the screen simplified.
		echo "$line"
		if [ "x${LOGGER}" != "x" ] ; then
			${LOGGER} -t ${SN} "$line"
		fi
...
f_InDMZ(){
   ...
   if [[ -n $ID_PUBIP ]] && [[ $ID_PUBIP != FAILURE ]]; then
      ANSWER=FALSE
      DMZ_RANGES="75.78.0.0/16 216.57.96.0/20 199.38.32.0/20 199.38.48.0/22 155.254.144.0/20 63.234.247.48/29"
...
f_RHELChangeHostname(){
   ...
   if [[ -z $RETCODE ]]; then
      ...
      ##20150514 - Alex - Added if/fi wrapper to stop overwriting localhost with $hostname
      if [[ $OLDHN != localhost ]] && [[ $OLDHN != localhost.localdomain ]]; then
...
#-----------------
# Function: f_replace_or_add
#-----------------
# Accepts a string to find, a string to replace
# it with, and a file to work on.  It then uses
# the "sed" command to replace those strings with
# the replacement string provided.
# It is possible to use "sed" search and replace
# constructs to keep some data.
# Examples
#   f_replace_or_add "net.ipv4.conf.all.accept_redirects.*=.*" \
#                    "net.ipv4.conf.all.accept_redirects = 0" \
#                    /etc/sysctl.conf
#   --> This will update the value to zero regardless of the value
#
#   f_replace_or_add "net.bridge.bridge-nf-call-ip6tables.*=.*" \
#                    "## REMOVED: &" /etc/sysctl.conf
#   --> This will add "## REMOVED: " before the line(s)
#
#-----------------
# Usage: f_replace_or_add s_FIND s_REPL s_FNAME
#-----------------
# Returns: SUCCESS
...
f_replace_or_add () {
	...
	if [[ -n `grep ${FINDSTR} ${FILENAM}` ]] ; then
		echo "Replacing '${FINDSTR}'."
		sed -i "s/${FINDSTR}/${REPLSTR}/g" ${FILENAM}
	else
		echo "Adding '${REPLSTR}' to ${FILENAM}."
		echo "${REPLSTR}" >> ${FILENAM}
	fi
}
